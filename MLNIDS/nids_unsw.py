"""
run_full_grid_unsw.py -- unified fair-pool full grid for UNSW-NB15, N_EVAL=500.
Saves results_full_unsw.csv (per-attack rows) + hyperparams.json.
"""
import os
import json
import glob
...
N_EVAL    = 500            # fair shared pool of 500 (avoids UNSW HopSkipJump hang)
...
FRESH           = True     # fresh run at N_EVAL=500
# =============================================================================
...
def save_hyperparams():
    hp = {
        ...
        "convergence": {"max_epochs": MAX_EPOCHS, "chunk": CHUNK,
                        ...
                        "monitor": "validation macro-F1 (10% stratified of train)"},
...
class _HSJTimeout(Exception):
    pass

def _hsj_chunk(clf, Xf):
    from art.attacks.evasion import HopSkipJump
    atk = HopSkipJump(classifier=clf, targeted=False, max_iter=HSJ_MAX_ITER,
                      max_eval=HSJ_MAX_EVAL, init_eval=HSJ_INIT_EVAL, batch_size=128,
                      verbose=False)
    return atk.generate(x=Xf).astype(np.float32)

def _hsj_once(clf, X, chunk=250, per_chunk_timeout=180):
    """HopSkipJump hardened against the nan-gradient DEADLOCK on sparse features.
    Chunks under a hard wall-clock timeout; a hung chunk is aborted and those
    samples fall back to clean input (attack recorded as failed)."""
    Xf = np.ascontiguousarray(X, np.float32)
    out = Xf.copy()
    for s in range(0, len(Xf), chunk):
        piece = np.ascontiguousarray(Xf[s:s+chunk], np.float32)

        def _handler(signum, frame):
            raise _HSJTimeout()
        old_handler = _signal.signal(_signal.SIGALRM, _handler)
        _signal.alarm(per_chunk_timeout)
        try:
            adv = _hsj_chunk(clf, piece)
            bad = ~np.isfinite(adv).all(axis=1)
            adv[bad] = piece[bad]
            out[s:s+chunk] = adv
        except _HSJTimeout:
            print(f"    [HSJ chunk {s}-{s+len(piece)} timed out ({per_chunk_timeout}s) -> "
                  f"samples treated as attack-failures]", flush=True)
            out[s:s+chunk] = piece
        finally:
            _signal.alarm(0)
