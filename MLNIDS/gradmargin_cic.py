# xai_grad.py -- the gradient-margin test that Section 4.7 says it has not done.
#
# Section 4.7 reports that the activation which spreads attribution over more
# features is also the less evaded one, and then states two reasons not to read
# that causally: (a) for an L_inf adversary the achievable change in the logit
# scales with the L1 norm of the input gradient, which GROWS as weight is spread
# over more features, so "breadth" is arguably the wrong quantity; and (b) the
# ordering is confounded with goodness of fit, because a better-fitting model has
# a larger decision margin. This script measures the quantity that settles both.
#
# For a locally linear model, an L_inf perturbation of size eps can change the
# margin by at most eps * ||g||_1, where g is the gradient of the margin with
# respect to the input. A flow therefore flips when
#
#       eps  >=  margin / ||g||_1  =:  eps_hat
#
# so eps_hat is the predicted perturbation needed to evade, per flow. It contains
# BOTH quantities at once: the margin (the fit confound) in the numerator and the
# gradient norm (which breadth of reliance drives) in the denominator. The
# fraction of originally-correct flows with eps_hat <= eps is a PREDICTED ASR,
# directly comparable to the measured ASR of the paper's tables.
#
# Restricting g to the features a realizable adversary controls gives the L2
# version of the same prediction, so the test also connects Section 4.7 to the
# realizability analysis of Section 4.5.
#
...
# and the capability masks -- is imported from your own scripts (nids_cic.py /
# nids_unsw.py via capability_cic.py / capability_unsw.py). Nothing is
# re-implemented here; this file trains the standard models and differentiates
# them.
#
# It runs NO attacks. Cost is 3 activations x 3 seeds of clean training per
# dataset, plus one backward pass over the evaluation pool: roughly the cost of
# fpr_only.py's standard half, and far cheaper than its PGD-AT half.
#
# Usage, in the folder that contains capability_cic.py / capability_unsw.py:
#     python xai_grad.py cic
#     python xai_grad.py unsw
# or set DS_DEFAULT below when running inside a notebook.
#
# Output: results_gradmargin_<ds>.csv, one row per (activation, seed, level, eps).
...
EPS         = [0.1, 0.3, 0.5]
OUT = f"results_gradmargin_{DS}.csv"
# -------------------------------------------------------------------------
...
def main():
    print(f"dataset={DS}  device={DEVICE}  depth={DEPTH}  levels={LEVELS}  "
          f"eps={EPS}  (no attacks)", flush=True)
    ...
    for act in ACTIVATIONS:
        for seed in SEEDS:
            ...
            # ASR is defined over originally-correct flows only (nids_cic._asr),
            # so the prediction is restricted to the same denominator.
            correct = m > 0
    ...
    print("\nThe test: does the ordering of pred_asr across activations match the "
          "ordering of the measured ASR at the same level and budget? If it does "
          "on both datasets, the margin-to-gradient ratio is a mechanism for the "
          "activation effect. If it does not, it is not, and the association of "
          "Section 4.7 stays an association.")
