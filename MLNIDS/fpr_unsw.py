# fpr_only_unsw.py -- false-positive cost of PGD adversarial training, UNSW-NB15.
# Identical to fpr_only.py except that the dataset is fixed to "unsw", so it
# cannot be run against CSE-CIC-IDS2018 by accident (notebooks ignore the
...
# (nids_unsw.py via capability_unsw.py). Nothing
...
# 2 defences. The standard models are quick; PGD-AT is roughly 8x a clean run.
#
# Usage, in the folder that contains capability_unsw.py:
#     python fpr_only_unsw.py
# or paste into a notebook cell and run. Reads unsw_processed.npz; no download.
#
# Output: results_fpr_unsw.csv, one row per (defence, activation, seed), with
...
# ----------------------------------------------------------------- CONFIG
DS = "unsw"                              # fixed: this file is the UNSW-NB15 run
DEPTH       = 4
FEATS       = 30
